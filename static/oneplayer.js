...
if ($('playerloggedname') == null && typeof(localStorage.player) == 'undefined'){
//stores new player information
user = prompt("Player Name: ");
...
else if (typeof(localStorage.player) == 'undefined' && $('playerloggedname') != null){
    loggeduser = $('playerloggedname').innerHTML;
     localStorage.player=JSON.stringify(
    {   
        name: loggeduser,
...
//if the player information has already been set, page loads info from localStorage
 else if(typeof(localStorage.player) != 'undefined'){
    p = JSON.parse(localStorage.player);
    if ($('playerloggedname')){p['name'] = $('playerloggedname').innerHTML;localStorage.player=JSON.stringify(p);}
    $('welcome').innerHTML = "<h1>Welcome back " + p['name'] + "!</h1>";
    } //end else
...
//cards are compared     
    ...
    var playerdata = JSON.parse(localStorage.player);
    playerdata['turns'] = count;
    playerdata['prog'] = $('hand').innerHTML;
    localStorage.player=JSON.stringify(playerdata);
}
